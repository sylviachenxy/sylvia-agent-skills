...
class EvidenceTests(unittest.TestCase):
    ...
    def test_text_cannot_prove_listening(self):
        self.assertFalse(self.answer(self.present(self.prepare(domain='listening')))['qualifies_independent'])
...
class FileTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='sh-en-store-test-')
    ...
    def test_user_text_cannot_inject_managed_markers(self):
        case = EvidenceTests()
        case.setUp()
        case.answer(case.present(case.prepare()), evidence='Text <!-- SH-EN:END --> preserved as text')
    ...
    def test_profiles_is_nonmutating_when_absent(self):
        ...
        proc = subprocess.run([sys.executable, str(SCRIPTS / 'tutor_store.py'), '--registry', str(reg), 'profiles'], capture_output=True, text=True)
    ...
    def test_cannot_init_inside_git_checkout(self):
        (self.root / '.git').mkdir()
        proc = subprocess.run([sys.executable, str(SCRIPTS / 'tutor_store.py'), '--registry', str(self.root / 'profiles.json'), '--profile', 'fake', 'init', '--vault', str(self.root)], capture_output=True, text=True)
    ...
    def test_explicit_registry_directory_alias_is_supported(self):
        ...
        proc = subprocess.run([sys.executable, str(SCRIPTS / 'tutor_store.py'), '--registry', str(alias / 'profiles.json'), 'profiles'], capture_output=True, text=True)
        self.assertEqual(proc.returncode, 0, proc.stderr)
    ...
    def test_registry_file_symlink_still_rejected(self):
        ...
        proc = subprocess.run([sys.executable, str(SCRIPTS / 'tutor_store.py'), '--registry', str(alias), 'profiles'], capture_output=True, text=True)
        self.assertNotEqual(proc.returncode, 0)

    def test_starter_items_all_freeze(self):
        items = json.loads((SCRIPTS.parent / 'assets/starter-items.json').read_text())
        for n, item in enumerate(items):
            self.state, _ = store.apply(self.state, 'prepare', {'event_id': f'SEED-{n}', 'item': item})
        self.assertEqual(len(self.state['items']), 8)

    def test_summary_starter_requires_real_compression(self):
        items = json.loads((SCRIPTS.parent / 'assets/starter-items.json').read_text())
        summary = next(item for item in items if item['item_id'] == 'START-SUMMARY-02')
        passage = summary['stem'].split('\n', 1)[1]
        self.assertGreaterEqual(len(passage.split()), 3 * 50)
        model = summary['key'].split('Possible 43-word version: ', 1)[1]
        self.assertEqual(len(model.split()), 43)
