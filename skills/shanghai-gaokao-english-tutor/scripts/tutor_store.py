...
DEFAULTS = {'exam_year': None, 'exam_session': 'unknown', 'grade': '', 'target_score': None,
            'timezone': 'Asia/Shanghai', 'daily_minutes': 30, 'feedback_language': 'zh-CN',
            'store_quotes': True, 'goal_id': None, 'corpus_path': None, 'baseline_note': ''}
BEGIN = '<!-- SH-EN:BEGIN '
END = '<!-- SH-EN:END -->'
...
def validate_settings(settings):
    ...
    require(settings['exam_session'] in {'unknown', 'january', 'june', 'both'}, 'Invalid exam session')
    require(settings['target_score'] is None or type(settings['target_score']) is int and 0 <= settings['target_score'] <= 150, 'Invalid target score')
    ...
    for name in ('grade', 'feedback_language', 'baseline_note'):
    ...
    if settings['corpus_path'] is not None:
        require(isinstance(settings['corpus_path'], str) and Path(settings['corpus_path']).is_absolute(), 'corpus_path must be an absolute local path')
        from curriculum_lookup import checked_corpus
        checked_corpus(settings['corpus_path'])
...
def apply(state, command, payload, now=None):
    ...
    if command == 'configure':
        ...
        require(item['source']['kind'] in {'original', 'user_material', 'official_verified'}, 'Invalid source kind')
        ...
        exact_keys(payload, ['event_id', 'presentation_id', 'session_id', 'occurred_at', 'support', 'result',
                             ...
                             'next_step', 'new_context', 'transfer_note', 'timer_verified', 'elapsed_seconds'], ['action_id'])
...
def view_content(state):
    data = context(state)
    lines = ['# 上海高考英语练习台', '', f"档案：{state['profile_id']} · revision {state['revision']}", '',
...
def main():
    ...
    try:
        ...
        if args.command == 'materials':
            ...
            root = vault / 'Learning/Shanghai-Gaokao-English' / profile
